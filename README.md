...
- 本項目旨在使用機器學習模型，通過分析客戶的特徵，預測客戶信用風險高低。
- 此項預測有助於銀行風險管理和授信決策。
...
1. **特徵選擇**
   計算了每個特徵與目標變量 Risk 之間的相關性，並選擇了相關性大於 0.1 的特徵作為模型的輸入特徵。
2. **數據平衡**
   使用了 SMOTE（合成少數類別過採樣技術）來處理數據集中的類別不平衡問題，使得高風險與低風險的樣本數量相等。
3. **特徵編碼與標準化**
   對數據進行了標準化處理，並通過多項式特徵變換來生成特徵交互項，從而提升模型的表現。
3. **特徵選擇**
   使用隨機森林模型對特徵進行選擇，保留了對模型預測貢獻最大的特徵。
3. **模型性能比較**  
   我們比較了多個模型的性能，並分析了其 ROC-AUC 分數。
3. **模型選擇與調優**  
   使用分層交叉驗證選擇最佳模型，並進行超參數調優和最終評估。
4. **最佳模型選擇**  
   使用了隨機森林（Random Forest）模型，並通過集成學習技術（堆疊分類器）來進一步提升模型的準確性。模型使用平衡的類別權重來處理類別不平衡問題。
5. **模型在測試集上的表現如下**
   Accuracy: 0.70
...
=======
最終，模型能夠預測72%的高風險客戶，有效解決了商業問題。
>>>>>>> 63bf9adde0bdb18d5c31759b78a7110ba44c6782

## 8. 結論
通過詳細的數據分析，我們發現了一些影響信用風險的重要因素，如年齡（age）、借款金額(Credit amount)、還款期限(Duration)、支票存款（Checking account）、儲蓄存款（Saving accounts）、住房（Housing）及資金用途（Purpose）。該模型表現良好，適合用於預測客戶的信用風險。通過選擇重要特徵、平衡數據以及使用隨機森林與集成學習技術，有效提升了模型的準確性，可以幫助銀行在貸款決策過程中進行有效的風險管理。

## 9. 未來工作
儘管模型表現不錯，但仍有一些改進空間。例如，可以進一步調整特徵工程過程，或者嘗試其他集成學習方法來進一步提升模型的表現。
